...
#[test]
fn test_client_example() {
    use fastcgi_client::{Address, ClientBuilder, Params};
    use std::{env, io};

    let script_filename = env::current_dir().unwrap().join("tests").join("php").join("index.php");
    ...
    assert_eq!(output.get_stdout().unwrap(), "Content-type: text/html; charset=UTF-8\r\n\r\nhello".as_bytes());
